...
function ensureCar24(){if(!data.cars.length)data.cars.push({id:id24(),name:'Mijn auto',plate:'',make:'',model:'',year:'',isDefault:true});}
function car24(id){ensureCar24();return data.cars.find(x=>x.id===id)||data.cars[0]}
...
function editCar24(id=''){const c=id?car24(id):{id:id24(),name:'',plate:'',make:'',model:'',year:'',isDefault:!data.cars.length};const name=prompt('Naam voertuig',c.name||'');if(name===null)return;const plate=prompt('Kenteken',c.plate||'');if(plate===null)return;const make=prompt('Merk',c.make||'');if(make===null)return;const model=prompt('Model',c.model||'');if(model===null)return;c.name=name.trim()||'Auto';c.plate=plate.trim();c.make=make.trim();c.model=model.trim();if(!id)data.cars.push(c);save();render()}
...
/* build v24 · FuelNest + Open-Meteo weather/air quality/pollen */

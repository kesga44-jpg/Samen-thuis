...
function stats24(){const e=filtered24(),cost=e.reduce((s,x)=>s+num24(x.total),0),lit=e.reduce((s,x)=>s+num24(x.liters),0),all=calc24('');const stations={};e.forEach(x=>stations[x.station||'Onbekend']=(stations[x.station||'Onbekend']||0)+num24(x.total));const top=Object.entries(stations).sort((a,b)=>b[1]-a[1]).slice(0,5);const cheapest=e.filter(x=>num24(x.pricePerLiter)>0).sort((a,b)=>num24(a.pricePerLiter)-num24(b.pricePerLiter))[0];return `<div class="period24">${[['week','Week'],['month','Maand'],['year','Jaar']].map(x=>`<button data-auto24-period="${x[0]}" class="${statsPeriod24===x[0]?'active':''}">${x[1]}</button>`).join('')}</div><div class="stats-grid24"><article><small>Uitgegeven</small><strong>${euro24(cost)}</strong></article><article><small>Brandstof</small><strong>${lit.toFixed(1)} L</strong></article><article><small>Gem. verbruik</small><strong>${all.l100?all.l100.toFixed(1):'—'} L/100</strong></article><article><small>Kosten / km</small><strong>${all.perKm?euro24(all.perKm):'—'}</strong></article><article><small>Jaarprojectie</small><strong>${euro24(statsPeriod24==='year'?cost:statsPeriod24==='month'?cost*12:cost*52)}</strong></article><article><small>Laagste literprijs</small><strong>${cheapest?euro24(cheapest.pricePerLiter):'—'}</strong></article></div><section class="card stats-card24"><h3>Uitgaven per tankstation</h3>${top.length?top.map(([n,v])=>`<div class="barrow24"><span>${esc(n)}</span><div><i style="width:${Math.max(6,v/top[0][1]*100)}%"></i></div><b>${euro24(v)}</b></div>`).join(''):'<p class="muted">Nog geen gegevens.</p>'}</section>`}
...
/* build v24.1 · RDW lookup + APK agenda/reminder + vehicle delete */
